 ...
 * @since    1.0.0
 */
class JsnExtFwChooserEasyBlogPostChooser extends JsnExtFwChooser
{
	/**
	 ...
	 * @var  string
	 */
	protected $query_params = 'view=blogs&tmpl=component';
	...
	protected function setResponse($content)
	{
			...
			if (strpos($table->getAttribute('class'), 'app-table') !== false)
			{
				...
				foreach ($links as $link)
				{
					if ($link->hasAttribute('data-eb-composer')
                        ...
                        || strpos($link->getAttribute('href'), 'javascript:void(0);') !== false)
					{
        ...
        // add body padding on the EasyBlog Items listing screen.
        $dom->getElementsByTagName('body')->item(0)->setAttribute('style', 'padding: 10px;');
