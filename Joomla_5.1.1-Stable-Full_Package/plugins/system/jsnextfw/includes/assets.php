...
class JsnExtFwAssets
{
	...
	protected static $assets = array(
		 ...
		 * 3rd-party assets.
		 */
		'base64' => 'assets/vendors/bs64.min.js',
	...
	/**
	 * Load Bootstrap 4 CSS framework.
	 *
	 * @param   boolean  $inline  Whether to print the script code to load the specified script immediately.
	 *
	 * @return  void
	 */
	public static function loadBootstrap($inline = false)
	{
		// Load Bootstrap 4 stylesheet.
		self::loadStylesheet(JSNEXTFW_URL . '/assets/vendors/bootstrap/css/bootstrap.min.css', $inline);

		// Load Bootstrap 4 scripts if the active admin template is Hathor.
		...
		self::loadInlineScript(
			';(function(api) {
				...
				array(
					'root' => JUri::root(true),
					'plugin' => JSNEXTFW_URL,
					'ajaxBase' => 'index.php?option=com_ajax&group=system&plugin=jsnextfw&format=json&' . JSession::getFormToken() . '=1',
					'chosenJs' => JsnExtFwHelper::isJoomla4() ? null : JUri::root(true) . '/media/jui/js/chosen.jquery.min.js',
					'chosenCss' => JsnExtFwHelper::isJoomla4() ? null : JUri::root(true) . '/media/jui/css/chosen.css'
				)) . ';
	...
	public static function loadEditionManager($callback = null, $component = null, $params = array())
	{
		...
		if (JFactory::getUser()->id > 0)
		{
			...
			if (!isset($loaded) || !isset($loaded[$component]))
			{
				...
				$save = JRoute::_("index.php?option=com_ajax&format=json&group=system&plugin=jsnextfw&context=account&{$save}", false);
